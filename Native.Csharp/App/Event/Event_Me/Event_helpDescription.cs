...
.计算 [算式]
范例：'.计算 (3+5)*2*π'
计算结果会作为环境变量'结果'储存。算式支持环境变量'结果'、'骰子'、'清点'。";
                    break;
                case "骰子":
...
范例：'.移除 怪兽卡区域 3'
移除多个序号时要注意，移除是即时生效的，需要考虑前面被移除的元素。";
                    break;
                case "抽牌":
...
字符串：变量名的后1个参数可输入。输入一个字符串，字符串会被赋值给变量，如范例所示。支持环境变量，参考'如果'指令。
...
表达式：表达式应为四则运算式子。表达式支持环境变量'骰子'、'清点'、'结果'，它们的值为上次'骰子'、'清点'、'计算'指令的结果。
