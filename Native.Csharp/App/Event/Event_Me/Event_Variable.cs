...
骰子指令全面升级；新增环境变量时点、缓存；'如果'指令现在支持包含判断；现在回复时会标注对象昵称，优先从玩家属性中获取；修复'如果'的子指令回复无法被屏蔽的BUG。";
    }
    /*
     * Ver1.2.0：
重构骰子指令，功能全面强化；新增管理员控制台，查看操作日志、修改黑名单；清理指令移入控制台内；新增环境变量时点、缓存；新增状态指令；新增建卡指令；新增记录指令；新增打包指令；新增教程指令；如果指令添加包含判断；属性加入默认昵称；现在回复时会标注对象昵称；升级框架以适配最新版酷Q。
